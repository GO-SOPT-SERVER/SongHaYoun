package sopt.org.fourthSixSeminar.external.client.aws;
...
public class S3Service {
    ...
    public List<String> uploadImages(List<MultipartFile> multipartFileList, String folder) {
      List<String> imageList=new ArrayList<>();
        for (MultipartFile multipart:multipartFileList
             ) {
            String fileName=createFileName(multipart.getOriginalFilename());
            ObjectMetadata objectMetadata = new ObjectMetadata();
            objectMetadata.setContentLength(multipart.getSize());
    ...
    private String getFileExtension(String fileName) {
        ...
        if (!fileValidate.contains(idxFileName)) {
            throw new BadRequestException(Error.NOT_FOUND_USER_EXCEPTION, Error.NOT_FOUND_USER_EXCEPTION.getMessage());
        }
